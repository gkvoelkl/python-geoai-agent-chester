...
def test_sanity_check_result(tmp_path):
    sample = write_building_sample(tmp_path)
    tools = tools_of(GeoValidationCapability(workspace=str(tmp_path)))
    r = tools["sanity_check_result"](
        path=str(sample["buildings"]), expected_geometry="Polygon"
    )
...
def test_detect_water_recovers_block_area(tmp_path):
    ...
    r = tools["detect_water"](
        green=str(green), nir=str(nir),
        mask_path="mask.tif", threshold=0.0, polygons_path="water.geojson",
    )
...
def test_render_map_basemap_selects_tiles(tmp_path):
    ...
    tools = tools_of(MapOutputCapability(workspace=str(tmp_path)))
    r = tools["render_map"](
        layers=[str(sample["buildings"])], output_path="m.html",
...
def test_render_map_wms_overlay_embeds_service(tmp_path):
    ...
    tools = tools_of(MapOutputCapability(workspace=str(tmp_path)))
    r = tools["render_map"](
        layers=[str(sample["buildings"])], output_path="wms.html",
        wms_url="https://example.org/wms", wms_layer="test:layer",
        wms_attribution="© Testdienst",
    )
    assert r["ok"] and r["wms"] == {"url": "https://example.org/wms",
                                    "layer": "test:layer"}
    html = Path(r["output"]).read_text()
...
def test_render_map_choropleth_classifies_by_column(tmp_path):
    ...
    tools = tools_of(MapOutputCapability(workspace=str(tmp_path)))
    r = tools["render_map"](
        layers=[str(sample["buildings"])], output_path="choro.html",
        column="true_height", scheme="NaturalBreaks", cmap="YlOrRd",
    )
    assert r["ok"]
...
def test_render_map_choropleth_missing_column_errors(tmp_path):
    ...
    tools = tools_of(MapOutputCapability(workspace=str(tmp_path)))
    r = tools["render_map"](
        layers=[str(sample["buildings"])], output_path="x.html", column="nope",
    )
...
def test_render_map_comma_joined_columns_routes_to_fields(tmp_path):
    ...
    tools = tools_of(MapOutputCapability(workspace=str(tmp_path)))
    r = tools["render_map"](
        layers=[str(sample["buildings"])], output_path="cj.html",
...
def test_render_map_single_columns_alias_is_choropleth(tmp_path):
    ...
    tools = tools_of(MapOutputCapability(workspace=str(tmp_path)))
    r = tools["render_map"](
        layers=[str(sample["buildings"])], output_path="sc.html",
        columns="true_height",
    )
    assert r["ok"], r.get("error")
    assert r["choropleth"]["column"] == "true_height"
...
def _write_raster(path, crs="EPSG:25832"):
    ...
    prof = dict(
        driver="GTiff", dtype="float32", count=1, width=40, height=30,
        crs=crs, nodata=-9999.0, transform=from_origin(500000, 5600030, 30, 30),
    )
...
def test_render_map_renders_raster_as_image_overlay(tmp_path):
    ...
    r = tools["render_map"](
        layers=[str(tif)], output_path="tri.html", column="TRI", cmap="RdYlGn",
    )
    assert r["ok"]
    html = Path(r["output"]).read_text()
...
def test_render_map_stacks_raster_under_vector(tmp_path):
    ...
    tools = tools_of(MapOutputCapability(workspace=str(tmp_path)))
    r = tools["render_map"](
        layers=[str(tif), str(sample["buildings"])], output_path="stack.html",
    )
...
def test_render_map_tolerates_param_aliases(tmp_path):
    ...
    r = tools["render_map"](
        layers=[str(sample["buildings"])], output_path="a.html", columns="[]",
    )
    assert r["ok"] and "choropleth" not in r

    # `layer` (singular) substitutes for `layers`.
    r2 = tools["render_map"](
        layer=str(sample["buildings"]), output_path="b.html",
    )
    assert r2["ok"] and r2["layers"] == [str(sample["buildings"])]

    # `columns` naming a real field drives the choropleth like `column` would.
    r3 = tools["render_map"](
        layers=[str(sample["buildings"])], output_path="c.html", columns="true_height",
    )
    assert r3["ok"] and r3["choropleth"]["column"] == "true_height"


def test_inspect_map_always_registered(tmp_path):
    # We assume the main model can see; inspect_map is always available (the
    # runtime signal — the model saying it sees no image — reveals otherwise).
    tools = tools_of(MapOutputCapability(workspace=str(tmp_path)))
...
def test_inspect_map_via_vision_model_without_config_notes(tmp_path):
    ...
    out = tools["inspect_map"](
        layers=[str(sample["buildings"])], via_vision_model=True
    )
