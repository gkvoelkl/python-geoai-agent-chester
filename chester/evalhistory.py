...
def per_model(records: list[dict]) -> list[dict]:
    """Aggregate pass-rate, mean tool-coverage and mean run time per model.
...
def format_report(records: list[dict], filter: str | None = None) -> str:
    ...
    if not records:
        ...
        return (
            "No eval history yet — run `uv run evals.py` or "
            "`uv run testprompt.py <id> --judge`."
        )
    ...
    lines = [
        ...
        "",
        "_By model:_",
        "| model | runs | pass | pass-rate | avg tools | avg time |",
        "|---|---|---|---|---|---|",
    ]
    for m in models:
        ...
        lines.append(
            ...
            f"{_fmt_dur(m['avg_duration'])}{timed} |"
        )
    lines += [
        "",
        "_Latest per test:_",
        "| test | verdict | cov | time | model | when |",
        "|---|---|---|---|---|---|",
    ]
    for r in tests:
        ...
        lines.append(
            ...
            f"{_fmt_pct(r.get('tool_coverage'))} | {_fmt_dur(r.get('duration_s'))} | "
